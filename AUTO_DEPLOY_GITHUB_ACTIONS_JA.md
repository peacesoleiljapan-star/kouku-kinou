...
今回の実装では、index.html や配布資料はコンテナへ bind mount しているため、GitHub 上で index.html を上書きした場合は NAS 側で git 更新が入るだけで反映されます。server.py、Dockerfile、compose.yaml を変えたときだけ docker compose build が走ります。
...
5. NAS 側の作業ツリーで git fetch と checkout を行います。
6. index.html のような差し替えだけなら rebuild せず、そのまま /api/health を叩いて即時反映します。
7. server.py や Dockerfile が変わった時だけ docker compose up --build -d を実行します。

## この構成が向いている理由

1. NAS をインターネットへ直接公開しません。
2. GitHub 側からの接続は Tailscale 内だけで閉じます。
3. index.html 更新時に Docker イメージを毎回作り直さないため、運用が軽くなります。
4. Actions 側は毎回 ephemeral node なので、長く残る踏み台サーバーが要りません。

## 事前に入れた実装

1. compose.yaml で index.html と配布用ファイル群を read-only bind mount しています。
2. server.py は index.html の mtime を見てテンプレートを再読込します。
...
2. Actions が起動
3. Synology で git 更新
4. /api/health へのアクセスで server.py が index.html を再読込
5. 数十秒以内に画面へ反映
...
1. /api/health が 200 を返すか確認
2. compose.yaml の bind mount が有効か確認
3. docker compose ps で kouku-kinou が起動中か確認

## 運用ルール

1. 非エンジニアの更新対象は index.html に寄せる
2. 本番側で直接ファイルを直さない
3. server.py や compose.yaml の変更はエンジニアが管理する
4. data フォルダと .env は NAS ローカルで保全する
5. Synology では Tailscale SSH ではなく DSM の SSH サーバーを使う